...
// student ID
const string Title = "2024-02 Object-Oriented Programming Pokemon Master";
...
public:
    ...
    string getlatestskilleffect() const;

    void takelatestskill(int skillIndex);
    void latestskilleffect(string effect);
...
public:
    ...
    string stringifyPokemonLastSkillData(Pokemon* pokemon);
...
string Pokemon::getlatestskill() const
{
    return latestskill;
}
string Pokemon::getlatestskilleffect() const
{
    return AttackEffect;
}
...
void Pokemon::takelatestskill(int skillIndex)
{
    if (skillIndex >= 0 && skillIndex < static_cast<int>(skills.size()))
    {
        latestskill = skills[skillIndex].getName();
    }
    else
    {
        latestskill = "Invalid Skill";
    }
}
void Pokemon::latestskilleffect(string effect)
{
    AttackEffect = effect;
}
...
string Renderer::stringifyPokemonLastSkillData(Pokemon* pokemon)
{
    string result = "Latest Skill: ";
    if (pokemon->getlatestskill().empty())
    {
        result += "-\n";
    }
    else
    {
        result += pokemon->getlatestskill();
        result += '\n';
        result += "It was ";
        result += pokemon->getlatestskilleffect();
        result += '.';
    }
    return result;
}
...
int main(int argc, char* argv[])
{
    ...
    while (player0.getPokemon()->getCurrentHP() > 0 && player1.getPokemon()->getCurrentHP() > 0)
    {
        ...
        vector<string> cell4 = rndr.generateCell(rndr.stringifyPokemonLastSkillData(player0.getPokemon()), (rndr.getWidth() / 2) + 1, 1);
        vector<string> cell5 = rndr.generateCell(rndr.stringifyPokemonLastSkillData(player1.getPokemon()), (rndr.getWidth() / 2) + 1, 1);
        ...
        int afterBaseDataHeight = static_cast<int>(headerCell.size());
        rndr.concatCells(cell4, headerCell, 0, afterBaseDataHeight - 1);
        rndr.concatCells(cell5, headerCell, rndr.getWidth() / 2, afterBaseDataHeight - 1);
        ...
        if (attacker->getPokemon()->getSkill(atkSelction)->getTriesLeft() > 0)
        {
            ...
            cout << "It was " << attacker->getPokemon()->getSkill(atkSelction)->getEffectivenessTypeName(defender->getPokemon()->getPokemonElement()) << "." << endl;
            cout << endl;

            string effect = attacker->getPokemon()->getSkill(atkSelction)->getEffectivenessTypeName(defender->getPokemon()->getPokemonElement());

            attacker->getPokemon()->takelatestskill(atkSelction);
            attacker->getPokemon()->latestskilleffect(effect);
        }
        else
        {
